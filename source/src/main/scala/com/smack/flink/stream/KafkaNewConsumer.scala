...
object KafkaNewConsumer extends App {
  val topics = "test"
  ...
  val cassdata = s0.map(new MapFunction[AccessLog, Tuple15[String, String, String, String, String, String, Int, Int, String, String, String, Int, Double, String, Date]] {
    def map(log: AccessLog) = {
      ...
      new Tuple15(log.namespace, log.serverName, log.remoteAddr, log.remoteUser, log.timeLocal, log.request, log.status.toInt, log.bodySize.toInt, log.httpReferer, log.httpUserAgent, log.httpXForwardedFor, log.requestLength.toInt, log.upstreamResponseTime.toDouble, log.upstreamAddr, create_time)
    }
  }).addSink(new CassandraTupleSink[Tuple15[String, String, String, String, String, String, Int, Int, String, String, String, Int, Double, String, Date]](
    "INSERT INTO mydb.nginx_base_log"
      + " (namespace, servername, remoteAddr, remoteUser, timeLocal, request, status, bodySize, httpReferer, httpUserAgent, httpXForwardedFor, requestLength, upstreamResponseTime, upstreamAddr, create_time)"
      + " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);",
    new ClusterBuilder() {
      def buildCluster(builder: Cluster.Builder): Cluster = {
        builder.addContactPoint("cassandra").withPort(9042).build()
